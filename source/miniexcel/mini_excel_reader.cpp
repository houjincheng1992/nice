...
public:
    ...
    bool open(const char* buffer, int32_t bufsize);
...
bool Zip::open(const char* buffer, int32_t bufsize)
{
...
bool ExcelFile::open(const char* buffer, int32_t bufsize)
{
    _zip = new Zip();

    if (!_zip->open(buffer, bufsize))
        return false;
